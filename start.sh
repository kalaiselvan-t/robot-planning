...
docker run --rm -it --name ros2 --network host --cap-add=NET_ADMIN --env="DISPLAY=:0" -v /tmp/.X11-unix:/tmp/.X11-unix:rw -v /dev/dri:/dev/dri pla10/ros2_humble:amd64 /bin/bash
